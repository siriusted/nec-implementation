# code from https://github.com/gsurma/cartpole
...
class ScoreLogger:
    def __init__(self, env_name, exp_name):
        ...
        self.SCORES_CSV_PATH = f"{os.getcwd()}/results/scores_{exp_name}.csv"
        self.SCORES_PNG_PATH = f"{os.getcwd()}/results/scores_{exp_name}.png"
        self.SOLVED_CSV_PATH = f"{os.getcwd()}/results/solved_{exp_name}.csv"
        self.SOLVED_PNG_PATH = f"{os.getcwd()}/results/solved_{exp_name}.png"
    ...
    def _save_png(self, input_path, output_path, x_label, y_label, average_of_n_last, show_goal, show_trend, show_legend):
        ...
        plt.plot(x[-average_range:], [np.mean(y[-average_range:])] * len(y[-average_range:]), linestyle="--", label="last " + str(average_range) + " episodes average")
